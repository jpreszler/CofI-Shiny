...
RSVP <- read_excel(params$rsvp, col_names = TRUE) %>% select(Email, Name, Response=`Will you be part of the procession for the 2018 Convocation?`) %>% filter(Response == "Yes") %>% separate(col=Name, into = c("FN.rsvp", "LN"), sep = "[:space:]", extra = 'merge') %>% mutate(Email = tolower(Email))
